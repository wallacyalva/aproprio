        ...
        <div class="page-container">
            ...
            <?php $this->load->view("template/header_menu_view", null) ?>                       
                ...
                <div class="page-title">                    
                    <h2><span class="fa fa-users"></span> Propostas para Ser Empreendedor</h2>
                </div>
                <!-- END PAGE TITLE -->
                <?php 
                                        ...
                                        <table class="table datatable table-striped table-condensed table-actions">
                                            ...
                                            ?>
                                                    <tr id="<?=$user->id?>">
                                                        ...
                                                        <button class="btn btn-primary btn-condensed" onclick="notyConfirmAdm<?=$user->id?>();"><i id="adm<?=$user->id?>" class="fa <?php if($user->rule==1){ echo "fa-check-square";} else { echo "fa-square-o";}?>"></i></button>
                                                        <button class="btn btn-info btn-condensed" onclick="notyConfirmEmp<?=$user->id?>();"><i id="emp<?=$user->id?>" class="fa <?php if($user->rule==5){ echo "fa-folder";} else { echo "fa-folder-open";}?>"></i></button>
            <script type="text/javascript">                                            
                    ...
                    noty({
                        ...
                        buttons: [
                                {addClass: 'btn btn-success btn-clean', text: 'Ok', onClick: function($noty) {
                                    $.ajax({
                                        url: '<?=ci_site_url("usuario/propostas") ?>',
                                        type: 'post',
                ...
                function notyConfirmEmp<?=$user->id?>(){
                    noty({
                        text: 'Realmente deseja mudar este usuário para empreendedor?',
                        layout: 'topRight',
                        buttons: [
                                {addClass: 'btn btn-success btn-clean', text: 'Ok', onClick: function($noty) {
                                    $.ajax({
                                        url: '<?=ci_site_url("usuario/propostas") ?>',
                                        type: 'post',
                                        data: {usuario_to_empreendedor: <?=$user->id?>},
                                        success: function() {
                                            $('#emp<?=$user->id?>').toggleClass('fa-folder fa-folder-open');
                                        }
                                    });
                                    $noty.close();
                                    noty({text: 'Usuário aalterado!', layout: 'topRight', type: 'success'});
